...
defineFeature(feature, test => {
    ...
    test('User can expand an event to see its details', ({ given, when, then }) => {
        ...
        when('the user clicks show events details', () => {
            EventWrapper = mount(<Event event={mockData[0]} />);
            expect(EventWrapper.state('showDetails')).toEqual(false);
            EventWrapper.find('.event-details').simulate('click');    // ERROR: Method “simulate” is meant to be run on 1 node. 0 found instead.
            //EventWrapper.find('showDetails').simulate('click');    // ERROR: Method “simulate” is meant to be run on 1 node. 0 found instead.
        });
